        ...
        <Reveal className="mt-9">
          <div className="overflow-hidden border border-ink bg-ink">
            <div className="border-b border-paper/20 px-4 py-2.5 label text-paper/55">
              src/policy_engine/policies/finance_agent.json
            </div>
...
"thresholds": {
 "privacy": {
 "routine":  { "warn": 0.10, "block": 0.30 },   // generate_text, draft_email
 "elevated": { "warn": 0.10, "block": 0.25 },   // send_email, update_crm
 "severe":   { "warn": 0.05, "block": 0.15 }    // delete_record, execute_payment
  }
},
"mandatory_human_review_actions": ["send_email", "update_crm",
 "delete_record", "execute_payment"],
"max_session_exposure": 0.5,
"pii_handling": "block",
"latency_budget_ms": 1000`}</code>
            </pre>
          </div>
        </Reveal>
      </Section>
      ...
      <Section id="measured">
        ...
        <div className="mt-10 grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { v: <Counter to={5} />, l: "Decision states, all reachable" },
            { v: <Counter to={50} />, l: "Tests, none of them mocking what they test" },
            { v: <Counter to={132} />, l: "Labelled eval cases, split dev and held out" },
            { v: "1.00", l: "Precision on the held-out split" },
          ].map((s, i) => (
